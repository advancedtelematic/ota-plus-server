...
export DEVICE_REGISTRY_DB_USER="$(echo $VDATA | jq -r .data.devices_db_username)"
export DEVICE_REGISTRY_DB_PASSSWORD="$(echo $VDATA | jq -r .data.devices_db_password)"
export DEVICE_REGISTRY_DB_URL="$(echo $VDATA | jq -r .data.devices_db_url)"
