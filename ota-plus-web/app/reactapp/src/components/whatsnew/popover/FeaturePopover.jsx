...
class FeaturePopover extends Component {
  renderButtons = () => {
    ...
    return _.map(actions, (action, index) => {
      ...
      return (
        validAction && (
          <button key={index} className={`col-xs-4 btn-${index === 2 ? 'primary' : 'bordered'}`} id={`button-${action}`} onClick={() => callback(action)}>
  ...
  render() {
    const { feature, step, totalSteps } = this.props;
    const initialStep = !step.previous;
    ...
    return (
      ...
      >
        ...
        <div className='popover-modal__headline'>
          <h4>{`Step ${step.order}/${totalSteps}`}</h4>
        </div>
        <div className='popover-modal__body'>
          <Content data={feature} />
        </div>
        <div className='popover-modal__actions'>{this.renderButtons()}</div>
      </Popover>
    );
  }
}
