...
class CreateModal extends Component {
  ...
  submitForm(type) {
    ...
    if (this.props.fileDropped) formData.append('file', this.props.fileDropped);
    else formData.append('file', this.refs.fileUpload.files[0]);
  ...
  render() {
    ...
    const { hardwareStore } = this.props.stores;
    const directorForm = (
      <Form onValid={this.enableButton.bind(this)} onInvalid={this.disableButton.bind(this)} onValidSubmit={this.submitForm.bind(this, 'director')} id='package-create-form'>
        <div className='row'>
          <div className='col-xs-6'>
            <FormInput label='Package Name' placeholder='Name' name='packageName' onInvalid={this.disableButton.bind(this)} id='add-new-package-name' />
          </div>
          <div className='col-xs-6'>
            <FormInput label='Version' name='version' id='add-new-package-version' onInvalid={this.disableButton.bind(this)} placeholder='Select version' />
          </div>
        </div>
        <div className='row'>
          <div className='col-xs-6'>
          ...
          <div className='col-xs-6'>
            <div className='row'>
              <div className='upload-wrapper col-xs-12'>
                {!fileDropped ? (
                  <a href='#' className='add-button' onClick={this._onFileUploadClick.bind(this)} id='choose-package'>
                ...
                <FormsyText type='text' name='fake-file' value={fileDropped ? fileDropped.name : this.fileName} style={{ display: 'none' }} required />
              </div>
            </div>
          </div>
        </div>
