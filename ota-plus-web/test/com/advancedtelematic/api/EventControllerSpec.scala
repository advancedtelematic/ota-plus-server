...
import org.genivi.sota.data.Device.{DeviceName, DeviceType, Id}
...
object MessagingData {
  ...
  val deviceCreatedMessage = new DeviceCreatedMessage(namespace, deviceName, deviceIdOpt, deviceType)
}

class EventControllerSpec extends PlaySpec with OneServerPerSuite with Results {

  protected def getDeviceSeenResponse(msg: DeviceSeenMessage): String = {
    "<html><body><script type=\"text/javascript\">parent.deviceSeen(" + Json.toJson(msg) + ");</script>"
  }

  protected def getDeviceCreatedResponse(msg: DeviceCreatedMessage): String = {
  ...
  val mockMsgSrc = new MessageBusConnection {
    ...
    : Namespace => Source[DeviceCreatedMessage, _] = {
      namespace => Source.single(MessagingData.deviceCreatedMessage)
    }
  }
  ...
  "EventController" should {
    "route DeviceSeenMessages from akka bus to client" in {
    ...
    "route DeviceCreatedMessages from akka bus to client" in {
